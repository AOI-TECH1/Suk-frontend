...
const Navbar = () => {
  ...
  // Logic: Calculate subtotal
  const subtotal = cart.reduce((acc, item) => acc + (item.price * item.quantity), 0);

  // Logic: Smart Name Display
    ...
    <header className="fixed top-0 inset-x-0 z-[100] px-4 pt-4 flex justify-center pointer-events-none font-sans">
      <nav className="bg-black/90 backdrop-blur-md rounded-full py-2 px-8 md:px-10 flex items-center justify-between text-white shadow-2xl border border-white/10 w-full max-w-6xl pointer-events-auto">
        ...
        <div className="hidden lg:flex items-center gap-6 text-[10px] font-black uppercase tracking-widest">
          <NavLink to="/" className={({ isActive }) => isActive ? "text-[#fbb03b]" : "text-gray-300 hover:text-white transition-colors"}>Home</NavLink>
          <NavLink to="/shop" className={({ isActive }) => isActive ? "text-[#fbb03b]" : "text-gray-300 hover:text-white transition-colors"}>Shop</NavLink>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" onMouseEnter={() => setShowAccountDropdown(true)} onMouseLeave={() => setShowAccountDropdown(false)}>
              <button className="flex items-center gap-1 text-gray-300 py-2 outline-none group">
                <User size={18} className="group-hover:text-[#fbb03b] transition-colors" />
                <div className="hidden sm:flex flex-col items-start leading-none">
                    <span className="text-[9px] font-black uppercase">{user ? `Hi, ${getDisplayName()}` : 'Sign In'}</span>
                </div>
                <ChevronDown size={10} className={`transition-transform duration-300 ${showAccountDropdown ? 'rotate-180' : ''}`} />
              </button>
            ...
            <div className="relative" onMouseEnter={() => setShowCartDropdown(true)} onMouseLeave={() => setShowCartDropdown(false)}>
              <Link to="/cart" className="relative text-gray-300 flex items-center py-2 group">
                <ShoppingCart size={18} className="group-hover:text-[#fbb03b] transition-colors" />
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-1.5 bg-[#fbb03b] text-black text-[7px] font-black rounded-full w-3.5 h-3.5 flex items-center justify-center border border-black group-hover:scale-110 transition-transform">
                    {cartCount}
                  </span>
                )}
              </Link>
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-2 pr-1 custom-scrollbar">
                    {cart.length > 0 ? (
                      cart.map((item) => (
                        <div key={item.id} className="flex items-center gap-3 p-2 bg-gray-50 rounded-xl group/item">
                          ...
                          <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-1.5 py-0.5">
                            <button onClick={() => updateQuantity(item.id, -1)} className="text-gray-400 hover:text-orange-500 transition-colors"><Minus size={10}/></button>
                            <span className="text-[9px] font-black min-w-[10px] text-center">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.id, 1)} className="text-gray-400 hover:text-orange-500 transition-colors"><Plus size={10}/></button>
                          </div>
                      ...
                      <div className="py-6 text-center">
                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Your bag is empty</p>
                      </div>
                    )}
                  </div>
                  ...
                  {cart.length > 0 && (
                    <div className="mt-4 pt-3 border-t border-gray-100">
                      ...
                      <button 
                        ...
                        className="w-full bg-black text-[#fbb03b] font-[900] text-[10px] uppercase py-3.5 rounded-xl hover:bg-zinc-800 transition active:scale-95 shadow-lg shadow-orange-100/20"
                      >
                        Proceed to Checkout
                      </button>
                    </div>
