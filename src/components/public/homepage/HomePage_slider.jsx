    ...
    <div className="relative w-full h-[90vh] overflow-hidden">
      {slides.map((slide, index) => (
        ...
        >
          ...
          {/* Overlay */}
          <div className="absolute top-1/2 left-20 -translate-y-1/2 bg-white/20 backdrop-blur-md p-8 rounded-2xl max-w-sm text-black shadow-lg">
            <h2 className="text-2xl font-bold">
              {slide.title}
            </h2>
            <p className="mt-2 mb-4">
      ...
      <div className="absolute bottom-5 w-full flex justify-center gap-2">
        {slides.map((_, index) => (
          <span
            key={index}
            onClick={() => setCurrent(index)}
            className={`w-9 h-1 rounded-full cursor-pointer ${
              index === current ? "bg-orange-500" : "bg-gray-300"
            }`}
          />
        ))}
